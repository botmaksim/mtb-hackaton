...
function BuildingGraphic({ building }) {
    ...
    if (isDecor) {
        return (
            <div className="w-16 h-16 bg-gradient-to-t border from-green-500 to-green-300 rounded-full shadow-2xl relative flex items-center justify-center">
    ...
    return (
        <div className={`bg-gradient-to-t rounded-t-xl border-2 shadow-2xl relative flex items-center justify-center flex-col group-hover:-translate-y-2 transition-transform
            ${building.level === 3 ? 'from-rose-600 to-pink-500 border-rose-800' : building.level === 2 ? 'from-orange-600 to-orange-400 border-orange-800' : 'from-amber-500 to-yellow-400 border-amber-700'}`} style={{ width: w, height: h }}
        >
