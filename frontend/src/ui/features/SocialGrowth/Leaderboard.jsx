    ...
    return (
        <ModalBase isOpen={isOpen} onClose={onClose} title="Друзья & Синдикаты">
            ...
            <div className="bg-white border text-gray-800 border-gray-100 rounded-3xl shadow-sm overflow-hidden divide-y divide-gray-50">
                {loading ? (
                    <div className="p-6 flex justify-center"><Loader2 className="animate-spin text-indigo-500" /></div>
                ) : filteredFriends.map((f, i) => (
                    <div key={f.id || i} className={`p-4 flex items-center gap-3 ${f.isMe ? 'bg-indigo-50/50' : ''}`}>
                        ...
                        <div className="flex-1 min-w-0">
                            <p className={`font-bold text-sm truncate ${f.isMe ? 'text-indigo-900' : 'text-gray-900'}`}>{f.name}</p>
                            <p className="text-xs text-gray-500 font-medium">{f.score.toLocaleString()} 🪙</p>
                        </div>
